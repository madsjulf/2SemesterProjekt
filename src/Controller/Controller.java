...
public class Controller {
    ...
    /**
     * Pre: Produktpris, salgsLinje, ProduktPrisListe og antal ikke er lig med null.
     * Metode der tilføjer et pantprodukt sammen med det givne produkt til en liste et givent antal gange.
     */
    public static void tilføjProduktMedProduktPant(ProduktPris produktPris, ObservableList<SalgsLinje> salgsLinje, ObservableList<ProduktPris> produktPrisListe, int antal) {
        ...
        for (SalgsLinje sl : salg.getSalgsLinjer()) {
            if (!salgsLinje.contains(sl)) {
                salgsLinje.add(sl);
                if (sl.getProduktPris().getProdukt().getProduktGruppe().getNavn().equals("Fustage")) {
                ...
                if (sl.getProduktPris().getProdukt().getProduktGruppe().getNavn().equals("Kulsyre")) {
    ...
    /**
     * Pre: salg ikke er = null
     * returnerer prisen for salget i klip.
     * Man kan ikke betale med klip og andre betalingsformer samtidigt.
     */
    ...
    /**
     * Pre: start og slut ikke begge er = null
     * returnerer alle færdige salg inden for en given periode, med den valgte og den afsluttende dato.
     */
    ...
    /**
     * Pre: start og slut ikke begge er = null
     * returnerer antal købte klip for en given periode.
     */
        ...
        if (start != null) {
            for (Salg salg : Storage.getSalgs()) {
                if (start.isBefore(salg.getSalgsDato()) || start.isEqual(salg.getSalgsDato())) {
                    if (salg.isSalgFærdigt()) {
                        for (SalgsLinje salgsLinje : salg.getSalgsLinjer()) {
                            if (salgsLinje.getProduktPris().getProdukt().getNavn().equals("Klippekort, 4 klip")) {
    ...
    /**
     * Pre: start og slut ikke begge er = null
     * returnerer antal brugte klip for en given periode.
     */
        ...
        if (start != null) {
            for (Salg salg : Storage.getSalgs()) {
                if (start.isBefore(salg.getSalgsDato()) || start.isEqual(salg.getSalgsDato())) {
                    if (salg.isSalgFærdigt()) {
                        for (SalgsLinje salgsLinje : salg.getSalgsLinjer()) {
                            if (salg.getBetalingsForm().equals("Klippekort")) {
                                antal = salgsLinje.getAntal();
